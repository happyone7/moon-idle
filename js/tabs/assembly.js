    ...
    if (!job) {
      ...
      slotsHtml += `<div class="slot-card">
        <div class="slot-card-header">
          <span class="slot-title">// 슬롯 ${idx + 1}</span>
          <span class="slot-state idle">대기 중</span>
        </div>
        <div style="font-size:10px;color:var(--green-mid);margin-bottom:4px;">${q.name} | ${asmCostStr}</div>
        <button class="btn btn-full btn-sm${canStart ? '' : ' btn-amber'}" onclick="startAssembly(${idx})" ${canStart ? '' : 'disabled'} style="font-size:10px;">
          ${canStart ? '[ START ]' : '[ 부품/자원 부족 ]'}
        </button>
      </div>`;
    } else if (job.ready) {
      ...
      slotsHtml += `<div class="slot-card" style="border-color:var(--green);">
        <div class="slot-card-header">
          ...
          <span class="slot-state ready">READY</span>
        </div>
        <div style="font-size:10px;color:var(--green-mid);margin-bottom:4px;">문스톤: +${ms}</div>
        <button class="btn btn-full btn-sm btn-amber" onclick="launchFromSlot(${idx})" style="font-size:10px;">[ LAUNCH ]</button>
      </div>`;
    } else {
      ...
      slotsHtml += `<div class="slot-card" style="border-color:var(--amber);">
        <div class="slot-card-header">
          ...
          <span class="slot-state busy">${fmtTime(remain)}</span>
        </div>
        <div class="prog-wrap"><div class="prog-fill amber" style="width:${pct}%"></div></div>
        <div style="font-size:10px;color:var(--green-mid);">${pct.toFixed(0)}%</div>
      </div>`;
    }
  });
